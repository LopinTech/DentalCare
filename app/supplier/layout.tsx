...
import { Truck, LayoutDashboard, ClipboardList, Package, Bell, X } from "lucide-react";
  ...
  return (
    <aside className="flex h-full w-64 flex-col border-r border-border bg-white">
      <div className="flex h-16 items-center justify-between px-5 border-b border-border">
        <Link href="/supplier" className="flex items-center gap-2 font-bold text-primary text-base">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-0.5">
          {NAV_ITEMS.map(({ label, href, icon: Icon }) => {
            const active = pathname === href || (href !== "/supplier" && pathname.startsWith(href));
            return (
              <li key={href}>
                <Link
                  href={href}
...
function SupplierHeader({ onMenuOpen }: { onMenuOpen: () => void }) {
  return (
    <header className="flex h-14 items-center justify-between border-b border-border bg-white px-4 lg:h-16 lg:px-6">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-foreground">Supplier Portal</span>
      </div>
      <div className="flex items-center gap-2">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" onClick={() => setSidebarOpen(false)}>
          ...
          <div className="absolute left-0 top-0 bottom-0" onClick={(e) => e.stopPropagation()}>
            <SupplierSidebar onClose={() => setSidebarOpen(false)} />
          </div>
        </div>
      )}

      <div className="flex flex-1 flex-col overflow-hidden">
        <SupplierHeader onMenuOpen={() => setSidebarOpen(true)} />
