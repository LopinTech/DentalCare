...
import { Briefcase, LayoutDashboard, Calendar, Bell, X } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { label: "Appointments", href: "/employee/appointments", icon: Calendar },
];

function EmployeeSidebar({ onClose }: { onClose?: () => void }) {
  const pathname = usePathname();
  return (
    <aside className="flex h-full w-64 flex-col border-r border-border bg-white">
      <div className="flex h-16 items-center justify-between px-5 border-b border-border">
        <Link href="/employee" className="flex items-center gap-2 font-bold text-primary text-base">
...
function EmployeeHeader({ onMenuOpen }: { onMenuOpen: () => void }) {
  return (
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-secondary">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <EmployeeHeader onMenuOpen={() => setSidebarOpen(true)} />
